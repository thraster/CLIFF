...
def main(args):
    ...
    if args.input_type == "image":
        ...
        if args.input_type == "video":
            ...
            result_filepath = osp.join(args.input_path, f"cliff_{args.backbone}.npz")
            # result_filepath = args.input_path +  f"\cliff_{args.backbone}.npz"
            # print(args.input_path)
            # print('folder',result_filepath)
        # get all image paths
    ...
    cliff = eval("cliff_" + args.backbone) # 使用 Python 的 eval() 函数动态地构造一个变量名，并将该变量的值赋给变量 cliff。
    ...
    if args.save_results:
        ...
        np.savez(result_filepath, imgname=img_path_list,
                 ...
                 detection_all=detection_all,pred_vertices = np.array(pred_vert_arr).reshape(-1,6890, 3))
        # np.save("smpl_v_cliff_slp", np.array(pred_vert_arr).reshape(-1,6890, 3))
# Loss = λ/SMPL*L/SMPL + λ/3D*L/3D + λ/2D*L/full 2D .
    if args.viz:
        print("--------------------------- Visualization ---------------------------")
...
# python demo.py --input_type image --ckpt data\ckpt\2024_06_04-23_48_50.pt --input_path test_samples/136.png --save_results --show_bbox --show_sideView
