...
def bbox_from_detector(bbox, rescale=1.1):
    ...
    """
    # 找到bbox的中心点 然后计算这个bbox的长边是200的多少倍，返回scale = n倍*1.1
    # center
...
def process_image(orig_img_rgb, bbox,
    ...
    crop_img = img.copy() # 只是切割的蒙版 crop_img/255后即可正常显示

    img = img / 255.
    mean = np.mean(orig_img_rgb/255, axis=(0, 1))
    std = np.std(orig_img_rgb/255, axis=(0, 1))
